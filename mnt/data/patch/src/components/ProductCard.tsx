      ...
      <div className="mt-2 space-y-1">
        ...
        {showItemPrice ? (
          <div className="flex flex-wrap gap-2 text-[11px]">
            ...
            {sanity.highDelivery ? (
              <span
                className="inline-flex items-center rounded-full bg-rose-100 px-2 py-0.5 text-rose-900 dark:bg-rose-900/30 dark:text-rose-200"
                title={sanity.highDeliveryReason || ''}
              >
                High delivery
              </span>
            ) : null}
        ...
        {!showTotal && showItemPrice ? (
          <div className="text-xs text-slate-500 dark:text-slate-400">
            Tip: use{' '}
            <Link className="underline" href={`/compare?q=${encodeURIComponent(p.title ?? '')}`}>
              Compare
            </Link>{' '}
